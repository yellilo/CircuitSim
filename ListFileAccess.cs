...
{
	private static readonly Type[] ModelConstructorParameters = [typeof(string)];
	private static readonly Dictionary<CircuitComponentType, Type> ComponentModelTypeMap = new()
	{
		{ CircuitComponentType.Ammeter, typeof(AmmeterModel) },
		{ CircuitComponentType.Cell, typeof(CellModel) },
		{ CircuitComponentType.FixedResistor, typeof(FixedResistorModel) },
		{ CircuitComponentType.Fuse, typeof(FuseModel) },
		{ CircuitComponentType.Voltmeter, typeof(VoltmeterModel) },
		{ CircuitComponentType.Wire, typeof(WireModel) }
	};
	...
	public static List<T> Load(string path)
	{
		...
		{
			// Return the empty list if nothing has been saved yet.
			return list;
		}
		...
		foreach (var id in ids)
		{
			...
			{
				CircuitComponentType componentType = (CircuitComponentType)Enum.Parse(typeof(CircuitComponentType), (string)file.GetValue(id, "Type"));

				objType = ComponentModelTypeMap[componentType];
			}

			var constructor = objType.GetConstructor(ModelConstructorParameters) ?? throw new NotImplementedException($"'{objType}' has no constructor that takes the following parameters: (string id)");
			var obj = (T)constructor.Invoke([id]);
