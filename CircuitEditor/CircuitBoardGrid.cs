...
public partial class CircuitBoardGrid : Node2D
{
  ...
  public override void _Draw()
  {
    ...
    // Draw vertical lines
    for (float x = Board.OriginPosition.X; x > 0; x -= Board.BoardScale)
    {
      DrawGridLine(new Vector2(x, 0), new Vector2(x, Board.Size.Y));
    }
    for (float x = Board.OriginPosition.X; x < Board.Size.X; x += Board.BoardScale)
    {
      DrawGridLine(new Vector2(x, 0), new Vector2(x, Board.Size.Y));
    }
    // Draw horizontal lines
    for (float y = Board.OriginPosition.Y; y > 0; y -= Board.BoardScale)
    {
      DrawGridLine(new Vector2(0, y), new Vector2(Board.Size.X, y));
    }
    for (float y = Board.OriginPosition.Y; y < Board.Size.Y; y += Board.BoardScale)
    {
      DrawGridLine(new Vector2(0, y), new Vector2(Board.Size.X, y));
    }
  }
